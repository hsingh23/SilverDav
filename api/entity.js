...
Entity.prototype = {
	...
	cellSize: null,
	DIRECTION: {
		UP: {name:'-up', axis: 'y', sign: -1},
		DOWN: {name:'-down', axis: 'y', sign: 1},
		LEFT: {name:'-left', axis: 'x', sign: -1},
		RIGHT: {name:'-right', axis: 'x', sign: 1},
		NONE: null
		},
	movement: null,
	facing: null,
	...
	setup: function setupEntity(descriptor, map, cellSize, loader, output) {
		...
		if (descriptor.hasOwnProperty('sprite')) {
			...
			metric = new Rectangle(
				0,
				0,
				this.location.size.width * this.cellSize.width,
				this.location.size.height * this.cellSize.height
			);
			...
			if (descriptor.hasOwnProperty('animation')) {
				this.sprite.playAnimation(
					descriptor.animation[1],
					descriptor.animation[2],
					descriptor.animation[3]
				);
	...
	draw: function drawEntity(mapPosition, clip) {
		this.sprite.boundingBox.point.set(
			mapPosition.x + this.location.point.x * this.cellSize.width,
			mapPosition.y + this.location.point.y * this.cellSize.height
		);
